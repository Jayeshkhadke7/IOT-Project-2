...
class _ObjectDetectorView extends State<NavigationScreen> {
  ...
  Future<void> _processImage(InputImage inputImage) async {
      ...
      if (objectCenterX < centerX - centerOffset) {
        ...
        if (object.labels.isNotEmpty) {
          ...
          if (label.confidence > 0.60) {
            ...
            _ttsService.setAwaitOptions();
            _ttsService.speak("${label.text} on your left .");
          }
        }
        ...
        if (object.labels.isNotEmpty) {
          ...
          if (label.confidence > 0.60) {
            _detectedObjectNames.add('${label.text} on your $location ');
            _ttsService.setAwaitOptions();
            _ttsService.speak("${label.text} on your right .");
          }
        }
      } else {
        if (object.labels.isNotEmpty) {
          ...
          if (label.confidence > 0.60) {
            _detectedObjectNames.add('${label.text} on your $location ');
            _ttsService.setAwaitOptions();
            _ttsService.speak("${label.text} in front of you .");
          }
        }
      }
    }
    ...
    // } else if (moveLeft) {
    //   _ttsService.setAwaitOptions();
    //   _ttsService.speak("${objects} detected on your left. ");
    // } else if (moveRight) {
    //   _ttsService.setAwaitOptions();
    //   _ttsService.speak("${objects} detected on your right. ");
    // }
